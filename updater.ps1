$content = Get-Content "c:\Users\Manu\Downloads\new-vedtam\new-vedtam\index.html" -Raw
...
# 1. Replace the block from <!-- Hero Partners Strip --> to <!-- CERT-In Subscribe Modal -->
$pattern = "(?s)<!-- Hero Partners Strip -->.*?<!-- CERT-In Subscribe Modal -->"
$replacement = "$techPartnersHtml`n    </div>`n  </section>`n  <!-- CERT-In Subscribe Modal -->"
$content = [regex]::Replace($content, $pattern, $replacement)

# 2. Insert CSS before </head>
$content = $content.Replace("</head>", "$css`n</head>")

# 3. Insert Our Clients before <section class="faq-section">
$content = $content.Replace('<section class="faq-section">', "$ourClientsHtml`n  <section class=`"faq-section`">")

# Write it back using [IO.File] to avoid Set-Content issues
[System.IO.File]::WriteAllText("c:\Users\Manu\Downloads\new-vedtam\new-vedtam\index.html", $content)
